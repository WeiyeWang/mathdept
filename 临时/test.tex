...
\begin{enumerate}[1.]
    ...
    \fourch{若$\sqrt[3]a>\sqrt[3]b$, 则$a>b$}{若$a>b,c>d$, 则$a-d>b-c$}{若$a>b>0,c>d>0$, 则$\dfrac ad>\dfrac bc$}{若$a>b>0,ac>bd$则$c>d$}
    ...
    (3)若$a>b,c>d$, 且a与d都是负数, 则$ac$\blank{50}$bd$.
    \item 若``$a>b,a-\dfrac 1a>b-\dfrac 1b$''同时成立, 则$ab$应满足的条件是\blank{50}.
    \item (1)已知$a>0,b>0$, 且$a\ne b$, 比较$\dfrac{a^2}b+\dfrac{b^2}a$与$a+b$的大小.
    (2)已知$0<\dfrac ab<\dfrac cd$, 比较$\dfrac b{a+b}$与$\dfrac d{c+d}$的大小.
    \item 若$x>y>1$, $0<a<1$, 则下列各式中正确的一个是\bracket{20}.
    \fourch{${x^{-a}}>{y^{-a}}$}{$(\sin a)^x>(\sin a)^y$}{${\log_{\dfrac 1a}}x<{\log_{\dfrac 1a}}y$}{$1+{a^{x+y}}>a^x+a^y$}
    \item (1)已知$a\in \mathbf{R}$, 比较$\dfrac 1{1+a}$与$1-a$的大小.
    (2)设$a>0$, $a\ne 1$, $t>0$, 比较$\dfrac 12\log_at$和$\log_a\dfrac{t+1}2$的大小.
    \item (1)已知$x>y>0$, 比较$\sqrt{\dfrac{{y^2}+1}{{x^2}+1}}$与$\dfrac yx$的大小.
    (2)已知$a$, $b$, $m$, $n$都是正实数, 且$m+n=1$, 比较$\sqrt{ma+nb}$和$m\sqrt a+n\sqrt b$的大小.
    二、一元二次不等式
    \item 解下列不等式:
    (1)$6x^2-5x-1>0$.						(2)$6x^2-5x-1>0$.
    (3)$5x^2-2x+3>0$.					(4)$9x^2+6x+1>0$.
    (5)$3x^2-4x+5<0$.
    \item 已知关于$x$的不等式$ax^2+bx+c<0$的解集是$\{x|x<-2x<-\dfrac 12\}$, 求$ax^2-bx+c>0$的解集.
    \item 已知集合$A=\{x|x^2+(a-1)x-a>0\}$, $B=\{x|(x+a)(x+b)>0\}$, $a\ne b$, $M=\{x|x^2-2x-3\le 0\}.$
    (1)若${{\complement }_U}B=M$, 求$a$, $b$的值.
    (2)若$-1<b<a<1$, 求$A\cap B$.
    (3)若$-3<a<-1$, 且$a^2-1\in \complement uA$, 求实数$a$的取值范围.
    ...
    \fourch{$M\cup N=M$}{$M\cap N=\{x|2<x<3\}$}{$M\cup N=R$}{$M\cap N=\{x|x<-2\}$}
    ...
    \fourch{$\{x|x<-\dfrac 43x>\dfrac 12\}$}{$\{x|-\dfrac 43<x<\dfrac 12\}.$}{$\{x|-\dfrac 12<x<\dfrac 43\}.$}{$\{x|x<-\dfrac 12x>\dfrac 43\}$}
    \item 当$a<0$时, 关于$x$的不等式$x^2-4ax-5a^2>0$的解集是\bracket{20}.
    \fourch{$\{x|x>5ax<-a\}$}{$\{x|x<5ax>-a\}$}{$\{x|-a<x<5a\}.$}{$\{x|5a<x<-a\}.$}
    \item 若$x$为实数, 则下列命题正确的是\bracket{20}.
    \fourch{$x^2\ge 2$的解集是$\{x|x\ge \pm \sqrt 2\}.$}{$(x-1)^2<2$的解集是$\{x|1-\sqrt 2<x<1+\sqrt 2\}.$}{$x^2-9<0$的解集是$\{x|x<3\}.$}{设$x_1,x_2$为$ax^2+bx+c=0$的两个实根, 且$x_1>{x_2}$, 则$ax^2+bx+c>0$的解集是$\{x|x_2<x<{x_1}\}.$}
    ...
    \fourch{$\begin{matrix}
       a>0,  \\ac\le \dfrac 14.  \end{matrix}$}{$\begin{matrix}
       a<0,  \\ac<\dfrac 14.  \end{matrix}$}{$\begin{matrix}
       a<0,  \\ac>\dfrac 14.  \end{matrix}$}{$\begin{matrix}}
       a<0,  \\ac<0.  \end{matrix}$
    ...
    \fourch{$-2<k<1$}{$-2\le k<-1$或$\dfrac 23<k\le 1$}{$k<-1$或$k>\dfrac 23$}{$-2<k<1$或$\dfrac 23<k<1$}
    \item 已知关于$x$的方程$(m+3)x^2-4mx+2m-1=0$的两根异号, 且负根的绝对值比根大, 那么实数$m$的取值范围是\bracket{20}.
    \fourch{$-3<m<0$}{$0<m<3$}{$m<-3$或$m>0$}{$m<0$或$m>3$}
    \item 若$\alpha ,\beta$芦是关于$x$的方程$x^2-(k-2)x+k^2+3k+5=0$($k$为实数)的两个实根, 则${{\alpha }^2}+{{\beta }^2}$的最大值等于\bracket{20}.
    ...
    \item (1)若关于$x$的二次方程$x^2-2mx+4x+2m^2-4m-2=0$有实根, 则其两根之积的最大值等于\blank{50}.
    (2)使关于$x$的方程$x^2-kx+2k-3=0$的两实根的平方和取最小值, 实数$k$的值等于\blank{50}.
    \item (1)若关于$x$的不等式$x^2-mx+n\le 0$的解集是$\{x|-5\le x\le 1\}$, 则实数$m=$\blank{50}, $n=$\blank{50}.
     (2)若关于$x$的不等式$ax^2+bx+1\ge 0$的解集是$\{x|-5\le x\le 1\}$, 则实数$a=$\blank{50},
    $b=$\blank{50}.
    (3)若关于$x$的不等式$ax^2+bx+2>0$的解集是$\{x|-\dfrac 12<x<\dfrac 13\}$, 则实数$a=$\blank{50}, $b=$\blank{50}.
    ...
    \item 若$a,b$均为大于1的正数, 且$ab=100$, 则$\lg a\cdot \lg b$的最大值是\bracket{20}.
    ...
    \fourch{最小值1}{最大值1}{最小值-1}{最大值-1}
    \item (1)若$x,y\in \mathbf{R}^+$, 且$x^2+y^2=1$, 则$x+y$的最大值是\blank{50}.
    (2)若$x+2y=2\sqrt 2a(x>0,y>0,a>1)$, 则$\log_ax+\log_ay$的最大值是\blank{50}.
    (3)若$x>1$, 则$2+3x+\dfrac 4{x-1}$的最小值\blank{50}, 此时$x=$\blank{50}.
    (4)若$x>0$, 则$x+\dfrac 1x+\dfrac{16x}{{x^2}+1}$的最小值是\blank{50}, 此时$x=$\blank{50}.
    (5)若正数$a,b$满足${a^2}+\dfrac{b^2}2=1$, 则$a\sqrt{1+{b^2}}$的最大值为\blank{50}, 此时$a=$\blank{50}, $b=$\blank{50}.
    \item (1)若$x>0$, 则$3x+\dfrac{12}{x^2}$的最小值是\blank{50}, 此时$x=$\blank{50}.
    (2)若$0<x<\dfrac 13$, 则$x^2(1-3x)$的最大值是\blank{50}, 此时$x=$\blank{50}.
    (3)若$xy>0$, 且$x^2y=2$, 则$xy+x^2$的最小值是\blank{50}.
    (4)${{\sin }^4}\alpha {{\cos }^2}\alpha$的最大值是\blank{50}, 此时$\sin \alpha =$\blank{50}, $\cos \alpha =$\blank{50}.
    (5)若正数$x,y,z$满足$5x+2y+z=100$, 则$\lg x+\lg y+\lg z$的最大值是\blank{50}.
    \item 若$\dfrac{x^2}4+{y^2}=x$, 则$x^2+y^2$有\bracket{20}.
    \fourch{最小值0, 最大值16}{最小值$-\dfrac 13$, 最大值0}{最小值0, 最大值1}{最小值1, 最大值2}
    ...
    \item (1)已知$a>1$且${a^{\lg b}}=\sqrt[4]2$, 求$\log_2(ab)$的最小值.
    (2)求函数$y=\dfrac{{x^4}+3{x^2}+3}{{x^2}+1}$的最小值.
    (3)求$f(x)=4{x^2}+\dfrac{16}{({x^2}+1)^2}$的最小值.
    ...
    \fourch{$\{x|0<x<\dfrac 14\}$}{$\{x|\dfrac 14<x<1\}$}{$\{x|0<x<1x>4\}$}{$\{x|\dfrac 14<x<4\}$}
    \item 若$a,b$都是小于1的正数, 且${a^{\log_b(x-5)}}<1$, 则$x$的取值范围是\bracket{20}.
    \fourch{$x>5$}{$x<6$}{$5<x<6$}{$x<5$或$x>6$}
    \item 不等式$\log_x\dfrac 45<1$的解集是\bracket{20}.
    \fourch{$\{x|0<x<\dfrac 45\}$}{$\{x|x>\dfrac 45\}$}{$\{x|\dfrac 45<x<1\}$}{$\{x|0<x<\dfrac 45\}\cup \{x|x>1\}$}
    \item 若函数$f(x)={\log_{a^2-1}}(2x+1)$在区间$(-\dfrac 12,0)$内恒有$f(x)>0$, 则实数$a$的取值范围是\bracket{20}.
    \fourch{$0<a<1$}{$a>1$}{$-\sqrt 2<a<-1$或$1<a<\sqrt 2$}{$a<-\sqrt 2$或$a>\sqrt 2$}
    \item 若不等式$\log_a(x^2-2x+3)\le -1$对一切实数都成立, 则$a$的取值范围是\bracket{20}.
    \fourch{$a\ge 2$}{$1<a\le 2$}{$\dfrac 12\le a<1$}{$0<a\le \dfrac 12$}
    \item 解下列关于$x$的不等式:
    (1)${\log_{\dfrac 12}}(3x-2)>{\log_{\dfrac 12}}(x+1)$:\blank{50}.
    (2)${\log_{\dfrac 13}}({x^2}-x-2)>{\log_{\dfrac 13}}(2{x^2}-7x+3)$:\blank{50}.
    (3)$\log_x\dfrac 12<1$:\blank{50}.
    (4)$\lg (x-\dfrac 1x)<0$:\blank{50}.
    (5)$\log_2|x-\dfrac 12|<-1$:\blank{50}.
    \item 已知集合$M=\{x|\log_3(x-m)>1\}$与$P=\{x|{3^{5-3x}} \ge \dfrac 13\}$满足$M\cap P\ne \varnothing$, 求实数$m$的取值范围.
    \item 解下列不等式:
    (1)$\log_8(2-x)+{\log_{61}}(x+1)\ge \log_4x$.	(2)${\log_{0.5}}(x+13)<{\log_{0.5}}(x^2-2x-15)$.
    (3)$\log_x(3\sqrt{x-1}-1)>1$.				(4)${\log_{x-1}}(6-x-x^2)>2$.
    (5)$\dfrac 1{\log_2(x-1)}<\dfrac 1{\log_2\sqrt{x+1}}$.			(6)$\dfrac{\log_3(1-\dfrac{3x}2)}{\log_9(2x)}\ge 1$.
    (7)${\log_{0.5}}({2^x}-1)\cdot {\log_{0.5}}({2^{x-1}}-\dfrac 12)\le 2$.
    \item 解关于$x$的不等式, 其中$a>0$, $a\ne 1$:
    (1)$\log_a(x+1-a)>1$.    (2)$\log_a(1-\dfrac 1x)>1$.
    (3) $\log_a(2x-1)>\log_a(x-1)$.    (4)${\log_a^2}x<{\log_x^2}a$.
    (5)${x^{\log_ax}}>\dfrac{{x^4}\cdot \sqrt x}{a^2}$.\blank{50}(6)$\sqrt{\log_ax-1}>3-\log_ax$.
    \item 已知$x$满足不等式$(\dfrac 12)}^{2x-4}}-{{(\dfrac 12)^x-{{(\dfrac 12)}^{x-2}}+\dfrac 14\le 0$, 且$y={\log_{\dfrac 1a}}({a^2}x)\cdot {\log_{\dfrac 1{a^2}}}(ax)$的最大值是0, 最小值是$-\dfrac 18$, 求实数$a$的值.
    ...
    \fourch{$\varnothing$}{$\{x|x<0\}$}{$\{x|0<x<1\}$}{$\{x|x<0x\ge 1\}$}
    \item 若$|a+b|<-c$, 则在``\textcircled{1} $a<-b-c$, \textcircled{2} $a+b>c$, \textcircled{3} $a+c<b$, \textcircled{4} $|a|+c<|b|$, \textcircled{5} $|a|+|b|<-c$''这五个式子中一定成立的个数是\bracket{20}.
    \fourch{1}{2}{3}{4}
    ...
    \fourch{$\{x|x\ne -1\}$}{$\{x|x>-1\}$}{$\{x|x<0x\ne -1\}$}{$\{x|-1<x<0\}$}
    \item 解下列不等式.
    (1)$x^2+|x|-6<0$:\blank{50}.
    (2)$x^2-2|x|-15>0$:\blank{50}.
    (3) $4<|1-3x|\le 7$:\blank{50}.
    (4)$|x-3|<x-1$:\blank{50}.
    (5)$\log_2|x-\dfrac 12|<-1$:\blank{50}.
    \item 若函数$y=\log_ax$在$x\in [ 2,+\infty  ]$上恒有$|y|>1$, 则实数$a$的取值范围是\blank{50}.
    156解下列不等式:
    (1)$|x^2-5x+10|>x^2-8$.		(2)$|x^2-4|\le x+2$.
    (3) $|x+1|<\dfrac 1{x-1}$. 			(4)$|x+2|-|x-3|<4$.
    (5) $|x+3|-|2x-1|<\dfrac x2+1$.
    \item (1)已知当$|x-2|<a$成立时, $|x^2-4|<1$也成立, 求正数$a$的取值范围.
        (2)已知关于$x$的不等式$|x-4|+|x-3|<a$在实数集$R$上的解集不是空集, 求正数$a$的取值范围.
    \item 解下列不等式:
    (1)${\log_{\dfrac 14}}|x|<{\log_{\dfrac 12}}|x+1|$ .		(2)$|\lg (1-x)|>|\lg (1+x)|$.
    (3)$|{\log_{\dfrac 13}}x|+|{\log_{\dfrac 13}}\dfrac 1{3-x}|\ge 1$.
    \item (1)求函数$f(x)=|x-\dfrac 12|-|x+\dfrac 12|$的最大值.
       (2)已知$|\lg x-\lg y|\le 1$, 则$\dfrac xy+\dfrac yx$的取值范围是\blank{50}.
    \item 解下列关于$x$的不等式:
    (1)$|{\log_{\sqrt a}}x-2|-|\log_ax-2|<2$.		(2)$|\log_ax|<|\log_a(ax^2)|-2$.
    \item 解下列关于z的不等式:
    (1) $|3^x-3|+9^x-3>0$.			(2)$|a^x-1|+|{a^{2x}}-3|>2(a>0)$.
    \item $\triangle ABC$三内角$A,B,C$对边长分别为$a,b,c$, 外接圆半径记作$R$. 求证:
    (1)$a^2+b^2+c^2\ge 2ab\cos C+2b\cos A+2ca\cos B$.
    (2)$(a+b-c)(b+c-a)(c+a-b)\le abc$.
    (3)$\dfrac 12(\dfrac 1a+\dfrac 1b+\dfrac 1c)\le \dfrac{\cos A}a+\dfrac{\cos B}b+\dfrac{\cos C}c<\dfrac 1a+\dfrac 1b+\dfrac 1c$.
    (4)$\dfrac 1{ab}+\dfrac 1{bc}+\dfrac 1{ca}\ge \dfrac 1{R^2}$.
    \item 已知$f(\log_ax)=\dfrac{a({x^2}-1)}{x({a^2}-1)}(0<a<1,x>0)$.
    (l)求$f(x)(x\in \mathbf{R})$的表达式, 并判断它的单凋性.
    (2)若$n\ge 2,n\in \mathbf{N}$, 求证: $f(n)>n$.
    \item 若正数$a,b,c$满足$a+b>c$, 求证: $\dfrac a{1+a}+\dfrac b{1+b}>\dfrac c{1+c}$.
    \item 求证:
    (1)$\dfrac 12\cdot \dfrac 34\cdot \dfrac 56\cdot \dfrac 78\cdots \cdot \cdot \dfrac{99}{100}<\dfrac 1{10}$.
    (2)$(1+\dfrac 13)(1+\dfrac 15)\cdots (1+\dfrac 1{2n-1})>\dfrac 1{\sqrt 3}\sqrt{2n+1}(n\in \mathbf{N},n>1)$.
    (3)$\dfrac{x_1^2}{{x_2}-1}+\dfrac{x_2^2}{{x_3}-1}+\cdots +\dfrac{x_{n-1}^2}{{x_n}-1}+\dfrac{x_n^2}{{x_1}-1}\ge n+x_1+x_2+\cdots +x_a$($x_1,x_2,\cdots ,x_n$都是大于1的实数).
    \item (1)若正数$a,b,c$满足$a+b+c$, 求证: $(1+a)(1+b)(1+c)\ge 8(1-a)(1-b)(1-c)$.
    (2)若$0\le a\le 1$, $0\le b\le 1$, $0\le c\le 1$,
    求证: $\dfrac a{1+b+c}+\dfrac b{1+c+a}+\dfrac c{1+a+b}+(1-a)(1-b)(1-c)\le 1$.
    \item 已知三棱锥的三条侧棱两两互相垂直, 且六条棱之和为定值$m$, 求证: 它的体积$V\le \dfrac{5\sqrt 2-7}{162}{m^3}$.
    \item 已知$a+b+c>0$, $ab+bc+ca>0$, $abc>0$求证: $a>0,b>0,c>0$.
    \item 求证: 任何面积等于1的凸四边形的周长及两条对角线的长度之和不小于$4+\sqrt 8$.
    \item 解下列关于$x$的不等式:
    (1)${2^{x+1}}+x>0$.			(2)$\dfrac{a(x-1)}{x-2}>1$.
    (3)$x^2+(a-4)x+4-2a>0$, 其中$-1\le a\le 1$.
    (4)$\dfrac x{\sqrt{1+{x^2}}}+\dfrac{1-{x^2}}{1+{x^2}}>0$.
    (5)$\dfrac{cx}{a{c^2}+b}-\dfrac x{2\sqrt{ab}}>{x^2}$, 其中$a,b,c\in \mathbf{R}$, 且$a>0$, $b>0$.
    \item 已知函数$f(x)=ax^2-c$满足$-4\le f(1)\le -1$, $-1\le f(2)\le 5$, 求证: $-1\le f(3)\le 20$.
    \item 已知关于$x$的方程$a{{\sin }^2}x+\dfrac 12\cos x+\dfrac 12-a=0$在$0\le x<2\pi$内有两个相异的实根, 求实数$a$的取值范围.
    \item 已知$|a|<1$, $|b|<1$, $|c|<1$, 求证:
    (l)$|1-abc|>|ab-c|$.\blank{50}(2)$a+b+c<abc+2$.
