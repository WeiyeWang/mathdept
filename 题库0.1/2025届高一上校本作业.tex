...
(6) 非负实数集.
\iffalse
\item 用适当的方法表示下列集合:(1) 能整除$10$的所有正整数组成的集合;
(2) 能被整除$10$的所有正整数组成的集合;
(3) 方程$x^2+2=0$的实数解组成的集合;
(4) 方程组$2x+y=0$, x-y+3=0的所有解组成的集合;
(5) 两直线$y=2x+1$和$y=x-2$的交点组成的集合.5.不定项选择题.
(1) 下面写法正确的是$									()$
\fourch{\ \emptyset\in\{a\}}{$(0$, 1)$\in\{0$, 1\} $}{ 1\in\{(0$, 1)$\}  }{ (0$, 1)$\in\{(0$, 1)$\}\ \ \     (E) 0\in\{0$, 1\} $(F) 0\notin\{0$, 1\} $}$(2) 集合$\{(x$, y)$|xy\geq0$, x\in\mathbf{R}, y\in\mathbf{R}\}是指$				()$
\fourch{第一象限内的所有点; $}{$第三象限内的所有点; $}{$第一象限和第三象限内的所有点; $}{$不在第二象限、第四象限内的所有点. $}
\item$若集合$M=\{0,2,3,7\}, P=\{x|x=ab,$\ a$,b\in$M, \ a\ne$b\}$. 用列举法写出集合$P$.
\item 已知集合$A={2$, a^2, a}, 且$1\in\ A$, 求实数 的值. $(B$组)$
\item$单选题:设集合$M={a|a=x^2-y^2$, x、$y\in\\mathbf{Z}}$, 下列数中不属于$M$的为$()$
\fourch{$3$}{$6$}{$9$}{12}
\item已知集合$A=\{x|x=a+\sqrt 2b,ab\in \mathbf{Z}\}$, 若$x_1x_2\in A$, 证明: $x_1x_2\in A$.
\item 已知集合$A=\{x|($k+1$)x^2+x-k=0\}$中只有一个元素, 求实数$k$的值.1.1.3集合之间的关系$-1
(A$组)$
\item$用符号``\subset''``$=$''或``\supset''填空:(1)$\{a\}______\{a$, b, c\};
(2)$\{a$, b, c\}\blank{50}{a, c};
(3)$\{1$, 2\}\blank{50}\{x|x^2-3x+2=0\}.
(4)$A=\{x|x^2-2x+1=0\}______B=\{x|x^2+2x-3=0\}$;
(5)$A=\{1$, 2\}\blank{50}B={x|x是$2$的正约数};
(6)$A=\{(x$, y)$|\mathrm{\vthicksp}xy>0\}______B=\{(x$, y)$|x>0$, y>0\}.
