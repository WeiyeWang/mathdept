...
\item 设函数$f(x)$和$x$都是定义在集合$\sqrt 2$上的函数, 存在$\sqrt 2x$, 都有$x$成立, 称函数$x$与$y$在$l$上互为``互换函数''.若函数$f(x)=a^x$($a>0a\ne 1)$与$g(x)=x+1$在集合$M$上互为``互换函数'', 则$a$的取值范围是\blank{50}.
二、选择题(本题共有4题, 满分20分, 每题5分)每题有且只有一个正确选项, 考生应在答题纸的相应位置, 将代表正确选项的小方格涂黑.
\item 函数$f(x)=2^x-\dfrac 1{2^x}$的图象关于\blank{50}\cdots\cdots\cdots\bracket{20}.
$\fourch{$原点对称\blank{50}$}{$直线$y=x$对称\blank{50}
$}{$直线$y=-x$对称\blank{50}$}{y$轴对称}
14 . 三国时期赵爽在《勾股方圆图注》中, 对勾股定理的证明可用现代数学表述为如下图所示, 我们教材中利用该图作为几何解释的是.\blank{50}\cdots\cdots\cdots\bracket{20}.
$\fourch{$如果$a>b$, $b>c$, 那么$a>c$
$}{$如果$a>b>0$, 那么$a^2>b^2$
$}{$对任意实数$a$和$b$, 有$a^2+b^2\ge 2ab$, 当且仅当$a=b$时等号成立
$}{$如果$a>b$, $c>0$那么$ac>bc$}
\item 若函数$f(x)=\begin{cases} \log__2xx\ge 1 \\ x+cx<1. \end{cases}$  则``$c=-1$''是``$y=f(x)^$在$R$上单调增函数''的
\blank{50}\cdots\cdots\cdots\bracket{20}.\blank{50}
$\fourch{$充分非必要条件.\blank{50}$}{$必要非充分条件.
$}{$充要条件.\blank{50}$}{$既非充分也非必要条件}
\item 若实数$x,y$满足$x^2+2\cos y=1$, 则$x-\cos y$的取值范围是\blank{50}\cdots\cdots\cdots\bracket{20}.
$\fourch{$ $[-1,+\infty)$\blank{50}$}{$  $[-1,\sqrt 3]$
$}{$ $[1,\sqrt 3+1)$\blank{50}$}{$  $[-1,\sqrt 3+1]$}
三、解答题(本大题共有5题, 满分76分)解答下列各题必须在答题纸的相应位置写出必要的步骤.
\item (本题满分14分, 第1小题满分8分, 第2小题满分6分)
 如图, 正四棱柱$ABCD-A_1B_1C_1D_1$的底面边长$AB=2$,
若异面直线$A_1A$与$B_1C$所成角的大小为$\arctan \dfrac 12$,
(1) 求$BD_1$与底面$ABCD$所成角的正切值;
(2) 求正四棱柱$ABCD-A_1B_1C_1D_1$的体积.
\item (本题满分14分, 第1小题满分7分, 第2小题满分7分)
...
\item (本题满分14分, 第1小题满分6分, 第2小题满分8分)
如图, 某班级墙上有一壁画, 最高点$A$离地面4米, 最低点$B$离地面2米, 某同学从距离墙$x(x>1)$米, 离地面高$a(1\le a\le 2)$米的$C$处观赏该壁画, 设观赏视角$\angle ACB=\theta$.
